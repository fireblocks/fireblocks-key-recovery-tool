...
class BlockfrostClient(CardanoClient):

    def __init__(self, blockfrost_api_key: str, mainnet: bool):
        if not mainnet:
            raise Exception("Blockfrost only support mainnet")

        self.api_client = BlockFrostApi(
            project_id=blockfrost_api_key,
            base_url=ApiUrls.mainnet.value,
        )

    def get_current_slot(self) -> int:
        slot = self.api_client.block_latest().slot
        return int(slot)

    def submit_tx(self, signed_tx: bytes) -> str:
        rand_f_end = ''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(20))
        tmp_file_name = f"ada_tx_{rand_f_end}.tx"
        print(f"Storing: {signed_tx.hex()} to temporary file: ")
        with open(tmp_file_name, "wb") as f:
            f.write(signed_tx)
        resp = self.api_client.transaction_submit(tmp_file_name)
    ...
    def get_utxos(self, addresses: List[CardanoAddress], native_only: bool) -> List[CardanoUTxO]:
        ...
        for addr in addresses:
            try:
                addr_utxos = self.api_client.address_utxos(addr.address, **{"gather_pages": True})
            except Exception as e:
                if len(addr.address) == 58:
                    continue
                else:
                    print(f"Unable to get utxos for address {addr}: {e}")
                    raise e
            addr_tokens = []
            for utxo in addr_utxos:
                utxo_outputs = utxo.amount
                lovelace = 0
                for output in utxo_outputs:
                    if output.unit != "lovelace":
                        ticker = self._get_asset_name(output.unit)
                        try:
                            addr_tokens.append(
                                CardanoToken(
                                    ticker,
                            ...
                            print(f"Unable to add token {ticker}: {e}")

                    else:
                        try:
                            lovelace += int(output.quantity)
                        except Exception as e:
                            print(f"Failed to add amount of output to UTXO: {e}")

                utxos.append(CardanoUTxO(
                    bytearray.fromhex(utxo.tx_hash),
    ...
    def _get_asset_name(self, asset: str) -> str:
        try:
            asset = self.api_client.asset(asset)
            return asset["name"]
        except Exception as e:
            print(f"Failed to get asset {asset}: {e}")
            return "UNKNOWN_TOKEN_NAME"


class CardanoGQLClient(CardanoClient):
...
class CardanoWallet:
    def __init__(self, fpub: str, account: int, min_address_index: Union[int, None],
                 max_address_index: Union[int, None], cardano_client: CardanoClient, mainnet: bool):
        if account < 0:
            raise Exception(f'Invalid account value of {account}')

        self.__fpub = fpub.encode('utf-8')
        self.__account = account
        self.__cardano_client = cardano_client
    ...
    def do_native_transfer(self, fpriv: str, to_address: str, net_amount: int,
        ...
        return self.__cardano_client.submit_tx(signed_tx)

    def __get_utxos_for_account(self) -> List[CardanoUTxO]:
        return self.__cardano_client.get_utxos(
    ...
    def __get_balance_for_addresses(self, addresses: List[CardanoAddress]) -> CardanoBalanceMap:
        ...
        for utxo in self.__cardano_client.get_utxos(addresses, True):
    ...
    def __get_ttl(self) -> int:
        return self.__cardano_client.get_current_slot() + TX_TTL_SECS
